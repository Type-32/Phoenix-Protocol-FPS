Shader "Simon Mercuzot/RedDot" {
	Properties {
		_Color ("Color", Color) = (1,1,1,1)
        _MainTex ("Base (RGB)", 2D)="white"{}

		[Space]
		_DotColor ("Red Dot Color(RGB) Brightness(A)", Color) = (1,1,1,1)
		_RedDotTex ("Red Dot Texture (A)", 2D) = "white" {}
		_RedDotSize ("Red Dot size", Range(0,100)) = 0.0
		[Toggle(FIXED_SIZE)] _FixedSize ("Use Fixed Size", Float) = 0
		_RedDotDist ("Red Dot offset distance", Range(0,500)) = 2.0
        ...
        Pass {
            CGPROGRAM
                ...
                fixed4 frag (v2f i): COLOR {
					...
					col.rgb += redDot * _DotColor.rgb * _DotColor.a;
					col.a += redDot * _DotColor.a;
                    return col;
                }
            ENDCG
		}
